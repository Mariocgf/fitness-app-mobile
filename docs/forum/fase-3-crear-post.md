...
- `src/components/features/forum/FlaggedTermsText.tsx` — preview read-only que resalta en rojo los términos
  marcados (los `TextInput` de RN no pintan rangos internos, ni en nativo ni en web).
...
- `app/(tabs)/community/new.tsx` — pantalla del form: `KeyboardAvoidingView` (`padding` solo en iOS),
  `keyboardShouldPersistTaps="handled"`, banner de términos marcados (chips + preview), CTA "Publicar" en el
  header. Éxito → `router.replace('/community/{id}')` (ve su post; volver va al feed).
