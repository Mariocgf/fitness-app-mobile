...
**Átomo.** Selector de duración **horas + minutos** con el control NATIVO del SO (`@react-native-picker/picker`): en iOS dos wheels girables con banda resaltada central (`zinc-800`) y divisor vertical; en Android dos dropdowns. Cada columna lleva su unidad (`h`/`min`) y los ítems van coloreados con el acento. Al ser nativo **no pelea con el responder system** del `ScrollView` padre (a diferencia de un wheel JS, que entraría en conflicto de gesto — ver `agent-implementation-lessons.md`). **El resumen total ("480 min") lo pone el consumidor**, no el átomo.

**Props:** `hours: number`, `minutes: number`, `onChange: (hours, minutes) => void`, `maxHours?` (default 12), `minuteStep?` (default 15 → 00/15/30/45), `accent?: 'rose' | 'lime' | 'amber' | 'mono'` (default `mono`), `wheelHeight?` (iOS, default 180).

**En uso:** `SleepFormView` (Duración del sueño, `accent="rose"`, `maxHours=12`). Pensado para reusarse a futuro en la duración de Meditación.

**Cuándo NO usar:** un único valor de un rango (usar `WheelPicker`), o cantidades con `−/+` (usar `QuantityStepper`).
...
| `RoutinePlanContent` | **Vista scrolleable del plan** (selector de días + cards de comidas) extraída de `NutritionPlanView` para **reusarse** en el tab Plan (Draft/activa) y en el detalle de un plan del listado (`/nutrition/routine/[id]`). Exporta también `PlanSkeleton` (esqueleto de carga días+comidas) y `PlanSkeletonItem`. Reutiliza `NutritionDaySelector` + `RoutineMealCard`. Props: `routine`, `onMealPress`. |
| `NutritionRoutineListCard` | **Card del listado "Mis planes"** (`/nutrition/routines`, dark `zinc`/`amber`). Espejo de `RoutineListCard` de Fitness: dos variantes según `isActive` — activa = card destacada con borde amber, badge "Activo" y CTA "Ver plan"; resto = fila compacta con `IconTile` (`restaurant-outline`), nombre, `estado • N días • fecha` y chevron. **No muestra kcal/macros**: `NutritionRoutineSummaryDto` solo trae id/name/status/dayCount/fechas. Props: `routine`, `onPress`. |

> La pantalla **"Mis planes"** (`app/(tabs)/nutrition/routines.tsx`, ruta pusheada desde el tab Plan) lista las rutinas con `useMyNutritionRoutines` (paginado, activa primero por el backend), tocar una card abre el **detalle** (`app/(tabs)/nutrition/routine/[id].tsx`) que reusa `RoutinePlanContent` y permite **activar** el plan (`POST /{id}/activate`). El detalle usa el snapshot del contexto para la activa/draft y `getNutritionRoutineById` (`GET /api/nutrition-routine/{id}`, misma forma que `/active`) para el resto; las macros/receta por comida se cargan en la pantalla de meal reutilizando `getRoutineMealDetail` (`GET /meals/{mealId}`). "Generar plan" dispara `generate()` y vuelve al tab Plan para revisar el draft.
...
| Componente | Descripción |
|------------|-------------|
| `HealthDashboard` | **Orquestador de la pantalla Salud.** Maneja loading/error (acento `rose-400`) y arma: título "Salud" + **`WellnessEntryCard`** (acceso a Bienestar) + **bloque clínico** (`ClinicalProfileCard` + `RegisterReadingCard` + `ClinicalReadingsEntryCard`) + divisor + `LastMeasurementCard` + `BodyEvolutionDashboardSection` + `MeasurementHistorySection`. El bloque clínico va agrupado arriba (decisión del usuario: separar lo clínico de lo corporal). Prop `onOpenWellness` navega a `/health/wellness`. |
...
| `BodyEvolutionDashboardSection` | **Sección "Evolución física"** del dashboard (heading limpio, sin subtítulo). Muestra **solo la métrica principal** (Peso primero por `sortMetrics`) con un `BodyMetricTrendCard` + CTA `rose-400` "Ver más métricas" (`chevron-forward`) que navega vía `onViewMore`. El CTA solo aparece si hay más de una métrica. Estados loading/error/vacío en `zinc`/`rose-400`. Props: `dashboard`, `isLoading`, `error`, `onRefresh`, `onViewMore`. |
| `BodyEvolutionView` | **Vista "Evolución física"** (`/health/evolution`, dark `zinc`/`rose-400`). Header back circular (`bg-zinc-800`) + título grande + subtítulo. `ScrollView` con **todas** las métricas (Peso primero) mapeadas a `BodyMetricTrendCard`. Estados loading/error/vacío `zinc`/`rose`. `paddingBottom` suma `TAB_BAR_HEIGHT` + `insets.bottom` (vive en `(tabs)`, tab bar nativo encima). Props: `dashboard`, `isLoading`, `error`, `onBack`, `onRefresh`. |
...
#### Bienestar — `src/components/features/health/wellness/`

Submódulo de Salud (dark-only `zinc`, acento `rose-400`). Contrato en `docs/wellness-api-frontend-contract.md`. Enums serializados como **strings** (`"Good"`, `"Water"`, etc.) — **a diferencia del submódulo Clínico, que usa números** (ver `agent-implementation-lessons.md`). El azul de la maqueta se tradujo a `rose-400` (regla de siempre). La capa de datos cubre **todo el contrato** (4 features + guías) en `wellness.service.ts`/`types/wellness.ts`. De las vistas están construidas: el dashboard (`WellnessView`), la **pantalla de detalle de Sueño** (`SleepView`, `/health/sleep`, en `wellness/sleep/`) y la **pantalla de detalle de Hidratación** (`HydrationView`, `/health/hydration`, en `wellness/hydration/`). El formulario "Nuevo registro" y el detalle por log de Hidratación, los detalles/formularios de Ánimo/Meditación y el catálogo de guías son fase futura.

| Componente | Descripción |
|------------|-------------|
| `WellnessView` | **Vista principal de Bienestar** (`/health/wellness`, rediseñada desde la maqueta). Header "Bienestar" + subtítulo, card **"Hoy"** (`WellnessGroupCard` con 4 `WellnessLogRow`: Sueño/Hidratación/Ánimo/Meditación; subtítulo derivado del registro de HOY o "Sin registro"), grid **"Registrar"** 2×2 de `WellnessRegisterCard`, y **"Actividad reciente"** (`WellnessGroupCard` sin título con el merge de las 4 features, o empty-state). Estados loading/error en `rose-400`. El resumen "Hoy" no usa endpoint de stats (no existe): se deriva filtrando la página 1 de cada feature por la fecha de hoy. `onOpenSleep` abre `/health/sleep` y `onOpenHydration` abre `/health/hydration` (ambos cableados en su fila "Hoy"). Ánimo/Meditación siguen con callbacks `onRegister*?` opcionales (fase futura). Props: `today`, `recentActivity`, `isLoading`, `error`, `onRefresh`, `onOpenSleep?`, `onOpenHydration?`, `onRegisterHydration?`, `onRegisterMood?`, `onRegisterMeditation?`, `onOpenActivity?`. |
| `WellnessGroupCard` | **Contenedora.** Card `zinc-900 rounded-3xl` con título opcional (ej. "Hoy") que envuelve filas e **inserta divisores `zinc-800` entre ellas automáticamente** (`React.Children` + separadores). La usan "Hoy", "Actividad reciente" y el "Historial" del detalle de Sueño. Props: `title?`, `children`. |
| `WellnessLogRow` | **Átomo de fila** reutilizado en "Hoy", "Actividad reciente" y el "Historial" de Sueño: `IconTile` (rose) + título + subtítulo (+ nota opcional) + chevron opcional. **Reutilizar SIEMPRE** en vez de copiar el bloque fila. `subtitle` acepta `ReactNode` (para resaltar parte con el acento, ej. la calidad). `note?` agrega una 3.ª línea tenue. `showChevron?` (default `true`) → pasar `false` cuando la fila no navega a ningún detalle. Props: `icon` (Ionicons), `title`, `subtitle` (`ReactNode`), `note?`, `onPress?`, `showChevron?`. |
| `WellnessRegisterCard` | **Card del grid "Registrar"** (mitad de ancho, `flex-1`). Tile de ícono rose + chevron arriba, título + subtítulo chico debajo. Props: `icon`, `title`, `subtitle`, `onPress?`. |
| `WellnessEntryCard` | **Card del dashboard de Salud** → módulo Bienestar. `IconTile` (`sparkles` rose) + título + subtítulo + chevron. Mismo patrón que `ClinicalReadingsEntryCard`. Props: `onPress`. |

**Detalle de Sueño — `src/components/features/health/wellness/sleep/`** (mismo patrón que `clinical/ClinicalReadingsView`):

| Componente | Descripción |
|------------|-------------|
| `SleepView` | **Vista de detalle de Sueño** (`/health/sleep`). Header (back circular + "Sueño" + subtítulo), `SleepRegisterCard`, `SleepLastLogCard` (si hay último registro) y **"Historial"** (`WellnessGroupCard` con `WellnessLogRow` por registro, **con chevron** cuando `onSelectLog` está cableado → abre `sleep-detail`) con paginación **"Ver más registros"**. `ScrollView` con `paddingBottom = insets.bottom + TAB_BAR_HEIGHT + 16` (vive en el Stack de `(tabs)/health`, el tab bar nativo va encima). Props: `logs`, `lastLog`, `hasMore`, `isLoading`, `isLoadingMore`, `error`, `onBack`, `onLoadMore`, `onRefresh`, `onRegister?`, `onSelectLog?`. |
| `SleepDetailView` | **Detalle de un registro guardado** (`/health/sleep-detail`). Header ("Detalle de sueño" + subtítulo) + card de fecha/hora (círculo `border-zinc-700` con calendario rose + `formatFullDate` + "Registrado a las HH:MM") + card **"Descanso"** (duración grande centrada `{h} h {min} min` rose, divisor, fila "Calidad … {label}" rose) + card **"Nota"** (solo si existe) + acción **"Eliminar registro"** (`trash-outline` rojo, `red-400`). **No** muestra la duración duplicada en minutos ("480 min") de la maqueta (pedido del usuario). `ScrollView` con `paddingBottom = insets.bottom + TAB_BAR_HEIGHT + 16`. Props: `log` (`SleepLogDto`), `isDeleting`, `onBack`, `onDelete`. |
| `SleepRegisterCard` | **Card "Registrar sueño"** (espejo de `RegisterReadingCard`): `IconTile` luna rose + título/subtítulo + enlace "Nuevo registro →". `onPress?` opcional: sin él queda placeholder deshabilitado (`opacity-60`); `sleep.tsx` ya lo cablea a `sleep-new`. Props: `onPress?`. |
| `SleepLastLogCard` | **Card "Último registro"**: duración en grande (`{h} h {min} min`, rose) \| calidad (rose), línea de fecha + "Registrado a las HH:MM" (`formatFullDate`/`formatCapturedTime`) y, si existe, la nota separada por divisor. Props: `log` (`SleepLogDto`). |
| `SleepFormView` | **Formulario "Registrar sueño"** (`/health/sleep-new`, dark-only zinc, acento rose-400, espejo de `ClinicalReadingFormView`). Header + cards: **Fecha** (`DateTimePicker` spinner iOS / default Android, `maximumDate=hoy`), **Duración** (`DurationWheelPicker` h+min, default 8 h + resumen "🌙 {total} min" rose), **Calidad** (`SegmentedControl` rose `solid`, 5 opciones de `SLEEP_QUALITY_LABELS`), **Nota** (`TextInput` multiline, contador `N/120`) e info banner. Valida en cliente la duración (1–720 min). CTA sólido `rose-400` "Guardar sueño". Props: `isSubmitting`, `submitError`, `onBack`, `onSubmit(AddSleepLogDto)`. |

**Detalle de Hidratación — `src/components/features/health/wellness/hydration/`** (mismo patrón que `wellness/sleep/`):

| Componente | Descripción |
|------------|-------------|
| `HydrationView` | **Vista de detalle de Hidratación** (`/health/hydration`). Header (back circular + "Hidratación" + subtítulo), `HydrationRegisterCard`, `HydrationTodayCard` y **"Historial"** (`WellnessGroupCard` con `WellnessLogRow` por registro: título `formatFullDate`, subtítulo "{ml} ml · {bebida}" con la bebida en rose, `note` = `formatCapturedTime` del `capturedAt`) con paginación **"Ver más registros"**. El **ícono de la 3.ª línea (reloj) de la maqueta se omite**: `WellnessLogRow.note` es texto plano sin ícono y no se extiende el átomo por una sola pantalla. `onRegister?`/`onSelectLog?` opcionales (fase futura → sin ellos la card de registro es placeholder y las filas no navegan). `ScrollView` con `paddingBottom = insets.bottom + TAB_BAR_HEIGHT + 16`. Props: `logs`, `lastLog`, `todayMl`, `hasMore`, `isLoading`, `isLoadingMore`, `error`, `onBack`, `onLoadMore`, `onRefresh`, `onRegister?`, `onSelectLog?`. |
| `HydrationTodayCard` | **Card "Hoy"**: total de ml de hoy en grande (rose `text-7xl` + "ml") + línea "Último registro: {ml} ml · {bebida}" (bebida en rose) o "Sin registros hoy." si no hay logs. `todayMl` se calcula sumando los logs de hoy ya cargados (no hay endpoint de stats). Props: `todayMl`, `lastLog` (`HydrationLogDto \| null`). |
| `HydrationRegisterCard` | **Card "Registrar hidratación"** (espejo de `SleepRegisterCard`): `IconTile` gota (`water`) rose + título/subtítulo "Cantidad y tipo de bebida." + enlace "Nuevo registro →". `onPress?` opcional: sin él queda placeholder deshabilitado (`opacity-60`). Props: `onPress?`. |
| `HydrationFormView` | **Formulario "Registrar hidratación"** (`/health/hydration-new`, dark-only zinc, acento rose-400, espejo de `SleepFormView`). Header + cards: **Fecha** (`DateTimePicker` spinner iOS / default Android, `maximumDate=hoy`), **Cantidad** (valor grande "{ml} ml" rose + `SegmentedControl` rose `solid` con presets 250/500/750/1000 + fila "Cantidad personalizada" que despliega un `TextInput` `number-pad` inline — NO un wheel, para evitar el conflicto de gesto de las lecciones), **Tipo de bebida** (lista con divisores: ícono + label de `BEVERAGE_LABELS` + radio `radio-button-on/off`, 5 opciones de `BEVERAGE_OPTIONS`) e info banner "Puedes registrar varias tomas durante el día.". Valida en cliente la cantidad (1–10000 ml). CTA sólido `rose-400` "Guardar hidratación". Props: `isSubmitting`, `submitError`, `onBack`, `onSubmit(AddHydrationLogDto)`. |

**Detalle de Ánimo — `src/components/features/health/wellness/mood/`** (mismo patrón que `wellness/sleep/`):

| Componente | Descripción |
|------------|-------------|
| `MoodView` | **Vista de detalle de Ánimo** (`/health/mood`). Header (back circular + "Ánimo" + subtítulo), `MoodRegisterCard`, `MoodLastLogCard` (si hay último registro) y **"Historial"** (`WellnessGroupCard` con `WellnessLogRow` por registro: título `formatFullDate`, subtítulo = nivel de `MOOD_LABELS` en rose, `note` = nota del registro, **con chevron** cuando `onSelectLog` está cableado → abre `mood-detail`) con paginación **"Ver más registros"**. `ScrollView` con `paddingBottom = insets.bottom + TAB_BAR_HEIGHT + 16`. Props: `logs`, `lastLog`, `hasMore`, `isLoading`, `isLoadingMore`, `error`, `onBack`, `onLoadMore`, `onRefresh`, `onRegister?`, `onSelectLog?`. |
| `MoodLastLogCard` | **Card "Último registro"**: nivel de ánimo en grande (`MOOD_LABELS`, rose `text-5xl`), línea de fecha + "Registrado a las HH:MM" (`formatFullDate`/`formatCapturedTime`), la nota (si existe) y la **escala de ánimo** (`SegmentedControl` rose `solid`, 5 niveles de `MOOD_LEVELS_ORDERED`) en **modo solo-lectura** (`onChange` no-op) con el nivel registrado resaltado. Props: `log` (`MoodLogDto`). |
| `MoodRegisterCard` | **Card "Registrar ánimo"** (espejo de `SleepRegisterCard`): `IconTile` `happy` rose + título/subtítulo "Selecciona tu estado y agrega una nota opcional." + enlace "Nuevo registro →". `onPress?` opcional: sin él queda placeholder deshabilitado (`opacity-60`). Props: `onPress?`. |
| `MoodFormView` | **Formulario "Registrar ánimo"** (`/health/mood-new`, dark-only zinc, acento rose-400, espejo de `SleepFormView` **sin** el campo de duración). Header + cards: **Fecha** (`DateTimePicker` spinner iOS / default Android, `maximumDate=hoy`), **Estado de ánimo** (`SegmentedControl` rose `solid`, 5 opciones de `MOOD_LEVELS_ORDERED`, default "Bien"), **Nota** (`TextInput` multiline, contador `N/120`) e info banner. No requiere validación numérica (el ánimo siempre es válido). CTA sólido `rose-400` "Guardar ánimo". Props: `isSubmitting`, `submitError`, `onBack`, `onSubmit(AddMoodLogDto)`. |
| `MoodDetailView` | **Detalle de un registro guardado** (`/health/mood-detail`, espejo de `SleepDetailView`). Header ("Detalle de ánimo" + subtítulo) + card de fecha/hora + card **"Estado de ánimo"** (nivel grande centrado rose + escala `SegmentedControl` solo-lectura) + card **"Nota"** (solo si existe) + acción **"Eliminar registro"** (`trash-outline` `red-400`). `ScrollView` con `paddingBottom = insets.bottom + TAB_BAR_HEIGHT + 16`. Props: `log` (`MoodLogDto`), `isDeleting`, `onBack`, `onDelete`. |
